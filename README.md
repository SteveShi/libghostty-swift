# libghostty-swift (Chinese Version)
...
5. **沙盒化 Shell 模拟 (ShellCraftKit)**：保留沙盒内进程内 Shell 命令模拟执行 of API 及存根，为未来扩展保留通道。
...
# libghostty-swift (English Version)
...
*Note: This package internally links the binary static library `GhosttyKit.xcframework` and requires the client application to link and embed the `libghostty-vt.dylib` dynamic library at runtime.*
